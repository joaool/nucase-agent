...
//   npm run db:seed:mssql -- --company=aurora --target=docker
...
//
// --target defaults to "full" (the two real local SQLEXPRESS databases,
// provisioned from the complete 1,695-table PRIEXPRESS schema — see decision
// 8) and includes buildLookupPrerequisites()'s bootstrap rows for
// ExerciciosCBL/Moedas/DocumentosVenda/SeriesVendas/DocumentosBancos, which
// those databases' real FK graph requires. Pass --target=docker for a
// database built from schema.mssql.sql alone (e.g. a Railway/local Docker
// mssql service): that schema only has the 7 confirmed tables and captures
// PRIMARY KEY only, never FOREIGN KEY (see decision 9) — none of those 5
// lookup tables exist there and none of their FKs are enforced, so the
// prerequisite block is skipped entirely rather than failing on "invalid
// object name". Everything else about the generated data is identical
// either way.
...
function buildProfileSql(profile: CompanyProfile, target: "full" | "docker"): string {
  ...
  const statements: string[] = [
    `-- Generated by db/seed.mssql.ts for company profile: ${profile.key} (target: ${target})`,
    ...
    "",
    // See the --target usage comment at the top of this file: schema.mssql.sql
    // (target "docker") has no FK constraints at all on these 7 tables, so
    // none of ExerciciosCBL/Moedas/DocumentosVenda/SeriesVendas/DocumentosBancos
    // are required — and none of them exist there to INSERT into.
    ...(target === "full" ? buildLookupPrerequisites() : []),
...
async function main() {
  ...
  if (companyArg !== "aurora" && companyArg !== "flamecon") {
    throw new Error("Usage: tsx db/seed.mssql.ts --company=aurora|flamecon [--target=full|docker]");
  }
  const targetArg = process.argv.find((a) => a.startsWith("--target="))?.split("=")[1] ?? "full";
  if (targetArg !== "full" && targetArg !== "docker") {
    throw new Error("Usage: tsx db/seed.mssql.ts --company=aurora|flamecon [--target=full|docker]");
  }
  const profile = companyArg === "aurora" ? AURORA : FLAMECON;
  const generatedSql = buildProfileSql(profile, targetArg);
